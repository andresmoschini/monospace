...
- [x] T001 Confirm the working tree is clean and on the implementation stage branch, opened with
      `cargo xtask spec stage 79 impl`
- [x] T002 Capture the demonstration before anything changes:
      `cargo run -p monospace-cli > /tmp/demo-before.txt`, and keep the file until T035 (SC-003,
      TE-007)
- [x] T003 Create `crates/monospace-diagram/Cargo.toml`: name `monospace-diagram`, the description
...
- [x] T004 Add `crates/monospace-diagram` to `[workspace].members` and
      `monospace-diagram = { path = "crates/monospace-diagram" }` to `[workspace.dependencies]` in
      `Cargo.toml`
- [x] T005 [P] Create `crates/monospace-diagram/src/lib.rs` with the crate-level `//!` rustdoc
      saying what the crate holds and pointing at `docs/diagram-model.md` (FR-004)
- [x] T006 [P] Add `"-p", "monospace-diagram"` to the `wasm` step's args in `xtask/src/main.rs`,
...
- [x] T007 Create `crates/monospace-diagram/src/shape.rs` with
...
- [x] T008 Add `pub enum Shape` with the three struct variants `Box`, `Line` and `Arrow` and exactly
...
- [x] T009 Add `pub(crate) fn draw(&self, surface: &mut impl Surface)` to `Shape` in
...
- [x] T010 Declare `mod shape;` and re-export `Endpoint` and `Shape` from
...
- [x] T011 [US1] Create `crates/monospace-diagram/src/diagram.rs` with `pub struct Diagram` holding
      ...
      research Q2, Q8)
- [x] T012 [US1] Implement `pub fn draw(&self, buffer: &mut Buffer)` in
...
- [x] T013 [US1] Declare `mod diagram;` and re-export `Diagram` from
      `crates/monospace-diagram/src/lib.rs`

### Tests for User Story 1

- [x] T014 [US1] Add the `#[cfg(test)] mod tests` buffer helper in
      ...
      picture (research Q7)
- [x] T015 [US1] Test repeatability in `crates/monospace-diagram/src/diagram.rs`: a diagram holding
      a box and a line, drawn twice into equal windows, produces equal buffers (TE-003, scenarios 1
      and 2)
- [x] T016 [US1] Test that an empty diagram leaves its buffer exactly as it was, in
      `crates/monospace-diagram/src/diagram.rs` (scenario 3)
- [x] T017 [US1] Test clipping in `crates/monospace-diagram/src/diagram.rs`: a box partly outside
      the window draws what falls inside and nothing else, with no error (TE-002, scenario 4)
- [x] T018 [US1] Test a box against `monospace_core::BoxShape` drawn directly with the same
      parameters, in `crates/monospace-diagram/src/diagram.rs` (TE-005, scenario 5)
- [x] T019 [US1] Test a line against `monospace_core::Line` the same way, in
      `crates/monospace-diagram/src/diagram.rs` (TE-005)
- [x] T020 [US1] Test an arrow against `monospace_core::Arrow` the same way, both endpoints
...
- [x] T021 [US2] Test the equivalence of the two orders in
...
- [x] T022 [US2] Test that the order matters in `crates/monospace-diagram/src/diagram.rs`: the same
...
- [x] T023 [US2] Test that a crossing is composition, not occlusion, in
...
- [x] T024 [US3] Add `monospace-diagram.workspace = true` to `crates/monospace-cli/Cargo.toml`,
      keeping the dependency on `monospace-core` for `Buffer`, `GlyphCatalog` and `render`
- [x] T025 [US3] Delete the private `StampMode` enum and its `From` impl from
      `crates/monospace-cli/src/description.rs`, and drop the `mode` field from all three
      `ShapeDescription` variants (FR-017)
- [x] T026 [US3] Point `Endpoint`'s conversion in `crates/monospace-cli/src/description.rs` at
      `monospace_diagram::Endpoint` instead of `monospace_core::Endpoint`
- [x] T027 [US3] Replace `ShapeDescription::draw` with a conversion into `monospace_diagram::Shape`
...
- [x] T028 [US3] Rewrite `Description::render` in `crates/monospace-cli/src/description.rs` to build
...
- [x] T029 [US3] Update the module rustdoc in `crates/monospace-cli/src/description.rs` and the
      ...
      `monospace-diagram`
- [x] T030 [P] [US3] Remove every `"mode"` field from `crates/monospace-cli/assets/demo.json` and
...
- [x] T031 [US3] Update the unit-test fixtures in `crates/monospace-cli/src/description.rs` to the
      format without `mode`, and delete `an_unrecognized_mode_fails_to_deserialize` with the field
      it checked
- [x] T032 [P] [US3] Update the fixtures in `crates/monospace-cli/tests/cli.rs` to the format
...
- [x] T033 [US3] Make the overlap test in `crates/monospace-cli/tests/cli.rs` assert against the
...
- [x] T034 Run the whole gate: `cargo xtask check` is green, including `wasm` on three crates
      (SC-004)
- [x] T035 Confirm SC-003 by observation: `cargo run -p monospace-cli > /tmp/demo-after.txt` and
      `diff /tmp/demo-before.txt /tmp/demo-after.txt` reports nothing (TE-007)
- [x] T036 Confirm the `wasm` step reaches the new crate: put
...
- [x] T037 Walk [quickstart.md](quickstart.md) end to end on a fresh `cargo build --workspace` and
      ...
      `mode` render
- [x] T038 Append the increment's entry to `docs/learning-log.md`: what was learned about Rust
      design, what was learned about working this way, and the TE-007 observation from T035 with
      what was compared (principle II)
- [x] T039 Tick the boxes in this file that the work completed, and open the implementation pull
