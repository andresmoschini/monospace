...
Issue 104 is repaired inside this feature, ahead of the rule and in a commit of its own, because it
lives in the derivation the rule replaces. Issues 105 and 106 are not part of it: the first moves
the whole connection into a shape of its own and follows this work, and the second brings the
replaced construction back as a rule a shape can choose.

## User Scenarios & Testing _(mandatory)_

### User Story 1 - Two endpoints facing away from each other are joined (Priority: P1)
...
correct pictures — it is a picture with a gap in it where a person asked for a connection.
...
Over the whole grid this is the sweep of
...
3. **Given** any of the arrangements above with the two endpoints moved further apart, **When** the
   ...
   runs.
4. **Given** every arrangement of the sweep, **When** each is rendered from both ends, **Then** no
   arrangement draws two heads with a gap between them except the ones Edge Cases names, and in each
   of those the two heads are orthogonally adjacent so nothing looks disconnected.
...
### User Story 3 - One rule, and the same rule everywhere (Priority: P3)

The person from User Story 1 wants to be able to predict a picture without rendering it. Two things
stand in the way today, and both are the bound rather than the ranking.

The first is a family that has a shape of its own. Feature 099 found 17 arrangements whose route
...
does not — User Story 1's own picture is one of them. ADR-0048 settles it: the route passes on the
side the arrow's own travel puts to its right, and coordinates grow rightward and downward, so
leaving `up` puts the larger `x` to the right.

**Why this priority**: neither is a gap in a picture, so neither blocks User Story 1's value. Both
are what makes the result "one simple rule rather than a pile of special cases for each geometry",
which is the requirement issue 103 states about the shape of the answer rather than about the
picture.

**Independent Test**: render the double-escape arrangement from both ends and count the bends in
each picture; render an arrangement whose two routes mirror and check which side each is on against
the direction its `from` endpoint leaves in.

**Acceptance Scenarios**:

1. **Given** the double-escape arrangement above, **When** it is rendered from either end, **Then**
   the route has four bends, and the two orders mirror each other.
2. **Given** one endpoint at `(0, 0)` leaving `up` and one at `(1, 2)` leaving `down`, **When** the
...
3. **Given** one endpoint at `(0, 0)` leaving `down` and one at `(0, 2)` leaving `down`, **When**
...
4. **Given** any arrangement of the sweep, **When** every route that ties on bends, length and
...
- **Both endpoints at one position.** The two heads fall on one cell and the `to` endpoint's is the
  one seen, exactly as feature 099 pinned it. Nothing here moves that.
- **A route that leaves the window.** A route that wraps around the outside reaches further than the
  ...
  any shape is clipped today; the window is the caller's. Twelve of the sweep's renderings gain a
  route that falls outside the sweep window, so they still look like two heads in the snapshot
...
- **FR-007**: No family of arrangements may keep a construction of its own. The double escape MUST
  be gone from the rule and from the code, and no route may take more than five runs.
...
- The sweep of ADR-0045 stays the standing check: 1856 renderings, reviewed once against the rule
  ...
  exemption.
- Each of the three user stories gets named tests of its own, separate from the snapshot, with their
...
- A test MUST show that the derivation's cost does not grow with distance, rather than the claim
  being made in prose alone.
...
- **SC-002**: 268 renderings gain a route, of which 256 are visible inside the sweep window and 12
  are clipped by it.
...
- **SC-006**: Every rendering of the sweep changed by this slice is accounted for: 306 in all, 16
  for User Story 2 and 290 for User Story 1 and 3 together, and the remaining 1550 are unchanged.
- **SC-007**: Deriving a route for two endpoints fifty cells apart costs what it costs for two
  endpoints one cell apart, measured rather than argued.
- **SC-008**: A person reading a description can say where its route runs without rendering it,
  including which side a mirrored route passes on and which of two cells holds a turn.
